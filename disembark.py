...
def  intalize_render_widebody():
    ...
    ax.set_ylim(top=-0.5)
    ax.set_title('Widebody Disembarking Model')
...
def two_basic():
    ...
    seating_plan = [[ [1,0,assign_luggage()] for _ in range(TWO_ROWS)] for _ in range(TWO_SEATS)]
    #Clear aisles
...
def aisle_take_below(row,fuck,me):
                ...
                for current_aisle in (range(TWO_SEATS)):
                    ...
                    #ewginsdaogvnadsklbvasj nwklsfnwdsf
                        ...
                        if total_move_possibilites > 0:
                            ...
                            #fuck knows what is happening here but it works so it stays
...
def wuhan():
    global THANOS_COEFFICIENT
    THANOS_COEFFICIENT = 0 #0, 0.3 0.5 or 0.7
    off_the_plane(back_to_front, ' 0: ')
    THANOS_COEFFICIENT = 0.3 #0, 0.3 0.5 or 0.7
    off_the_plane(back_to_front, '0.3: ')
    THANOS_COEFFICIENT = 0.5
    off_the_plane(back_to_front, '0.5: ')
    THANOS_COEFFICIENT = 0.7
    off_the_plane(back_to_front, '0.7: ')
