...
def steffen_bofa_method():
    ...
    for _ in range(N_TEST_CASES):
        ...
        for row in range(1,NUM_ROWS+1):
            for seat in range(-3,4):
                #naughty boy
                if not is_not_a_naughty_boy() and seat != 0:
                    steffenPerfected[random.randrange(0,4)].append([row,seat,assign_luggage(),0])
    ...
    print(sum(test_cases)/len(test_cases))  
...
def naughty_boy_sensitivity():
    global NAUGHTY_BOY_COEFFICIENT
    nbsensitivity = []
    ...
    for i in range(0,41):
        ...
        nbsensitivity.append(steffen_bofa_method())
        
        
    rows.append(nbsensitivity)    
    print(nbsensitivity)
